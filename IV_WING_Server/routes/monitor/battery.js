...
router.get('/', function(req, res, next) {
	let memberVerifyTaskArray = [
		...
		(connection, callback) => {
	        ...
	        left join linker on non_linker.user_linker = linker.linker_id where non_linker.user_name is null order by linker_battery, user_name";
		...
		(connection, emptyData, callback) => {
            ...
            from (select * \
	            from (select * \
	            	...
	            	left join iv on records.record_iv = iv.iv_id) non_user \
	            left join user on non_user.record_user = user.user_id) non_linker \
	        left join linker on non_linker.user_linker = linker.linker_id where non_linker.user_name is not null order by linker_battery, user_name";
